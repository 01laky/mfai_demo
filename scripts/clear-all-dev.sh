...
if [ -f many_faces_mailer/docker-compose.tls-smoke.yml ]; then
  (cd "$ROOT/many_faces_mailer" && compose -f docker-compose.tls-smoke.yml -p mf-mailer-tls-smoke down -v --remove-orphans 2>/dev/null) || true
fi
...
_AI_RM=(be-demo-dev be-demo-seq be-demo-api fe-demo-dev fe-demo-proxy fe-demo-prod admin-demo-dev admin-demo-prod seq seq-dev postgres-dev pgadmin-dev redis-dev dozzle-dev elasticsearch-dev search-worker-dev push-worker-dev mailer-worker-dev elasticsearch-tls-smoke search-worker-tls-smoke push-worker-tls-smoke mailer-worker-tls-smoke)
if [[ "$CLEAN_AI" -eq 1 ]]; then
  _AI_RM+=(ai-demo-dev)
fi
...
remove_prefixed_volumes() {
  ...
  local candidates
  candidates=$(docker volume ls -q 2>/dev/null | grep -E '^(mfai_demo_|many_faces_main_|many_faces_database_|many_faces_redis_|many_faces_logger_|many_faces_backend_|many_faces_portal_|many_faces_admin_|many_faces_ai_|many_faces_elastic_|many_faces_push_|many_faces_mailer_|mf-search-tls-smoke_|mf-push-tls-smoke_|mf-mailer-tls-smoke_|db_demo_|redis_demo_|logger_demo_|be_demo_|fe_demo_|admin_demo_|ai_demo_)' || true)
...
_CONTAINER_RE='^(be-demo-dev|fe-demo-dev|fe-demo-proxy|admin-demo-dev|postgres-dev|pgadmin-dev|redis-dev|seq-dev|dozzle-dev|be-demo-seq|be-demo-api|elasticsearch-dev|search-worker-dev|push-worker-dev|mailer-worker-dev|elasticsearch-tls-smoke|search-worker-tls-smoke|push-worker-tls-smoke|mailer-worker-tls-smoke)$'
if [[ "$CLEAN_AI" -eq 1 ]]; then
  _CONTAINER_RE='^(be-demo-dev|fe-demo-dev|fe-demo-proxy|admin-demo-dev|ai-demo-dev|postgres-dev|pgadmin-dev|redis-dev|seq-dev|dozzle-dev|be-demo-seq|be-demo-api|elasticsearch-dev|search-worker-dev|push-worker-dev|mailer-worker-dev|elasticsearch-tls-smoke|search-worker-tls-smoke|push-worker-tls-smoke|mailer-worker-tls-smoke)$'
fi
...
_BAD_VOL_GREP='^(mfai_demo_|many_faces_main_|many_faces_database_|many_faces_redis_|many_faces_logger_|many_faces_backend_|many_faces_portal_|many_faces_admin_|many_faces_ai_|many_faces_elastic_|many_faces_push_|many_faces_mailer_|mf-search-tls-smoke_|mf-push-tls-smoke_|mf-mailer-tls-smoke_|db_demo_|redis_demo_|logger_demo_|be_demo_|fe_demo_|admin_demo_|ai_demo_)'
...
_NET_GREP='^(many_faces_main_dev-network|mfai_demo_dev-network|many_faces_database_db-network|many_faces_redis_redis-network|many_faces_backend_be-demo-network|many_faces_portal_fe-demo-network|many_faces_admin_admin-demo-network|many_faces_elastic_elastic-network|many_faces_push_default|many_faces_mailer_default|mf-search-tls-smoke_tls-smoke-net|mf-push-tls-smoke_tls-smoke-net|mf-mailer-tls-smoke_tls-smoke-net|db_demo_db-network|redis_demo_redis-network|be_demo_be-demo-network|fe_demo_fe-demo-network|admin_demo_admin-demo-network)$'
