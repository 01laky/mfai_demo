# MFAI Demo — application context

Single place to answer **what this system is**, **who uses which surface**, and **how the pieces hang together**. For deep mechanics, use [`docs/README.md`](./docs/README.md). Submodules retain their own READMEs.
...
| **Realtime + AI** | SignalR hubs (chat, messenger, notifications, …); optional **SendToAi** flows through **`ai_demo`** over gRPC. |
...
Every **meaningful REST read/write** on `fe_demo` for tenant content MUST be coherent with **`FaceConfigProvider.selectedFace`**, which MUST stay aligned with the URL **`/:lang/:faceIndex/...`** whenever the segment is an actual face index (see hooks that sync pathname → face selection).
...
`gridSchema.items[]` carries **`componentType`** (string enum mirrored in **`admin_demo` `GridComponentType`**).
...
| Source | Policy |
|--------|--------|
| **PostgreSQL seed / demo tenants** (`be_demo` seeders, seeded users/faces, `demo-users-and-passwords.md`) | **Keep** while we need scripted demos until product says drop them. Coordinate removal with docs + teardown scripts. |
