...
| [moderation-content-prompt-injection-defense-agent-prompt.md](./prompts/moderation-content-prompt-injection-defense-agent-prompt.md) | **Agent prompt:** defense-in-depth so **untrusted** album/blog/reel text and URLs cannot hijack **`ReviewContent`** / future moderation LLM; sanitization, instruction–data separation, validation, heuristics, red-team corpus, metrics; **superadmin chat excluded**. |
