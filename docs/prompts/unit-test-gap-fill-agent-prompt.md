...
**Purpose:** Systematically **add and strengthen automated tests** across **`many_faces_portal`**, **`many_faces_admin`**, **`many_faces_backend`**, **`many_faces_ai`**, and **repo scripts** where coverage is thin or risk is high — **without** making **component render / DOM snapshot testing** the primary strategy. Use this document as a **copy-paste agent brief** for one or a series of PRs.
...
Replace ✓ / N/A honestly. For every **N/A**, add **one short reason** (e.g. “no `many_faces_portal` files changed”). For a **full line-by-line** execution list aligned to every subsection of this prompt, complete **§15** as well (same ✓ / N/A discipline).

| §          | Expectation                                                                       | Status (✓ / N/A) | If N/A, reason |
| ---------- | --------------------------------------------------------------------------------- | ---------------- | -------------- |
| **§1**     | Inventory                                                                         |                  |                |
| **§2**     | P0→P2 discipline                                                                  |                  |                |
| **§3**     | `many_faces_portal` — entire section read; subsections implemented **or** N/A per row below |                  |                |
| **§3.1**   | Routing / URL                                                                     |                  |                |
...
| **§4**     | `many_faces_admin` — entire section read; **§4.1–§4.4** implemented **or** N/A          |                  |                |
| **§5**     | `many_faces_backend` + **§5.1–§5.3**                                                         |                  |                |
| **§6**     | `many_faces_ai` + **§6.1**                                                              |                  |                |
| **§7**     | `scripts/`                                                                        |                  |                |
...
**(required)** **§9** and **§10** must be ✓ (no N/A) for every PR. **§15**: every **subsection §15.1–§15.16** must be **✓** (all applicable inner checkboxes done) or **N/A** once with reason (e.g. **`§15.5` N/A — no `many_faces_portal` changes**); do not leave subsection headers ambiguous.

**(required — if whole app untouched)** You may collapse **§3** / **§4** / **§5** / **§6** / **§7** to a **single** row **N/A** with reason **`no {many_faces_portal|many_faces_admin|many_faces_backend|many_faces_ai|scripts} changes`** instead of filling each subsection row — but **any app that _is_ touched** must use the granular rows for that app.
...
## 3. `many_faces_portal` — recommended targets (**required** read for any `many_faces_portal` test PR; no render-first)

**(required)** If this PR changes **`many_faces_portal`** tests or testable modules, read **§3.1–§3.9** and satisfy **§0.2** rows for `many_faces_portal`. If **`many_faces_portal` is untouched**, mark **§3** (and **§3.1–§3.9** or the collapsed **§3** row per **§0.2** rule) **N/A** with reason **`no many_faces_portal changes`**.
...
## 4. `many_faces_admin` — recommended targets (**required** read for any `many_faces_admin` test PR)

**(required)** If this PR changes **`many_faces_admin`** tests or testable modules, read **§4.1–§4.4** and satisfy **§0.2** rows for `many_faces_admin`. If **`many_faces_admin` is untouched**, mark **§4** **N/A** with **`no many_faces_admin changes`**.
...
## 5. `many_faces_backend` — where to add tests (**required** read for any `many_faces_backend` test PR; incremental)
...
**(required)** If this PR changes **`many_faces_backend`** tests or production code under test, read **§5** (including **§5.1–§5.3**) and satisfy **§0.2** rows for `many_faces_backend`. If **`many_faces_backend` is untouched**, mark **§5** **N/A** with **`no many_faces_backend changes`**.
...
### 5.2 Security-sensitive test data (**required — P0** for every `many_faces_backend` test PR)
...
## 6. `many_faces_ai` (**required** read for any `many_faces_ai` test PR)

**(required)** If the PR does **not** touch **`many_faces_ai`**, mark **§6** / **§6.1** as **N/A** in **§0.2** with reason **`no many_faces_ai changes`**.

**Targets:**

- `pytest` modules — expand beyond smoke: gRPC error status, invalid protobuf, timeout behavior (where mocked).

**(required)** If adding network tests, mark **hermetic** (no real external calls) or skip in CI with explicit env gate documented in `many_faces_ai/README.md`.

### 6.1 Pytest structure (**required** for any non-trivial `many_faces_ai` test addition; **N/A** only for single-file smoke with no new fixtures/markers — state that in PR)
...
- **Vitest:** colocate `__tests__/*.test.ts` next to module or mirror `src/foo.ts` → `src/foo.test.ts` **only** if the repo already uses that pattern; **prefer existing** `many_faces_portal` / `many_faces_admin` layout.
...
- Match existing test file language (**English** descriptions in `many_faces_backend`; match SPA test file language already in use).
...
- [ ] `many_faces_portal`: `yarn test` (and `yarn validate` if the PR touches linted code).
- [ ] `many_faces_admin`: `yarn test` and `yarn validate`.
- [ ] `many_faces_backend`: `dotnet test BeDemo.Api.Tests` (or solution test command used in CI). For a **subset**, use **`dotnet test --filter "FullyQualifiedName~YourClass"`** or xUnit **traits** (`[Trait("Category","Security")]`) if the repo adopts them — document the filter in PR.
- [ ] `many_faces_ai`: `pytest` (or documented subset, e.g. **`pytest -m "not integration"`**).
...
- `many_faces_portal/src/api/__tests__/facePathRouting.test.ts`
- `many_faces_portal/src/acl/__tests__/permissions.test.ts`
- `many_faces_portal/src/utils/__tests__/routeTranslations.test.ts`
- `many_faces_admin/src/hooks/api/__tests__/usePagesApi.test.ts` (and siblings)
- `many_faces_backend/BeDemo.Api.Tests/JwtValidationEdgeTests.cs`, `RefreshTokenEdgeCaseTests.cs`, `SecurityEdgeCaseTests.cs` — structure for edge naming and arrange/act/assert
- `many_faces_backend/BeDemo.Api.Tests/IntegrationTestSeed.cs` (or current equivalent) — **central** user/role/face IDs for new tests (**§5.1**)
...
| PR # | Scope                                      | Apps                 |
| ---- | ------------------------------------------ | -------------------- |
| 1    | Pure routing + JWT + env                   | `many_faces_portal`            |
| 2    | `useWallHostViewer` + one hand-written API | `many_faces_portal`            |
| 3    | Hook parity (`usePageTypesApi` + gaps)     | `many_faces_admin`         |
| 4    | BE service/controller gap close            | `many_faces_backend`            |
| 5    | `many_faces_ai` gRPC edge + script smoke         | `many_faces_ai`, `scripts` |
...
**(required)** Work this section **top to bottom** before requesting review. For each bullet: **✓** done, or **N/A** with a **one-line reason** in the PR (same rules as **§0.2**). If an entire **subsection** (e.g. **§15.6**) does not apply, mark its header **N/A** once (`no many_faces_portal changes`, etc.) and skip inner boxes.

> **Reference implementation (2026-04-10):** `many_faces_portal` gained pure **`buildLocalizedLinkPath` / `buildLocalizedNavigateTarget`**, **`authSessionActions`** (shared login/logout/refresh/read-token logic) with **`authSessionActions.test.ts`** + **`authKeys`** coverage, **`jwtUtils`**, **`authTokenRequest`**, **`buildLocalized*`** tests; **`many_faces_admin`** mirrors **`authSessionActions`** + tests and **`usePageTypesApi.test.ts`**. Same pass added **`wallHostViewerLogic`** tests, **`env` / `collectEnvValidationErrors`** tests, **`computeAlbumGridLayout`** tests, **`profileQueryKey` + `profileApi`** tests, **ACL catalog `it.each`** over **`ALL_ACL_PERMISSION_KEYS_SORTED`** (fe + admin), **`StoriesControllerTests`** (401 without auth), **`many_faces_ai`** `pytest` + **`conftest.py` / `pytest.ini` / gRPC marker** wiring. **Still open for future PRs:** full **`useWallHostViewer` hook** `renderHook` coverage (Vitest **`node`** pool today), **`SignalR` / `EventSource`** handler + unmount tests, **`scripts/`** CI gates, and optional **§15.3** inventory artefacts.
...
### 15.5 §3 — `many_faces_portal` (skip whole subsection **N/A** if no `many_faces_portal` changes)
...
### 15.6 §4 — `many_faces_admin` (skip **N/A** if no `many_faces_admin` changes)
...
### 15.7 §5 — `many_faces_backend` (skip **N/A** if no `many_faces_backend` changes)
...
### 15.8 §6 — `many_faces_ai` (skip **N/A** if no `many_faces_ai` changes)

- [ ] `pytest` beyond smoke: gRPC errors, invalid protobuf, timeouts (mocked) where in scope.
- [x] Network tests **hermetic** or CI skip + **`many_faces_ai/README.md`** env gate documented.
...
- [x] `many_faces_portal`: `yarn test` (+ `yarn validate` if linted files touched) **or** **N/A**.
- [x] `many_faces_admin`: `yarn test` + `yarn validate` **or** **N/A**.
- [x] `many_faces_backend`: `dotnet test` (full or `--filter` / traits documented) **or** **N/A**.
- [x] `many_faces_ai`: `pytest` (subset documented if used) **or** **N/A**.
