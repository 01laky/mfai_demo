...
## Upload path safety (SHV2 BE-U2 / BE-U4)

| Item | Implementation |
| ---- | ---------------- |
| **BE-U2** | `UploadLimits.AvatarMaxBytes` (30 MB) and `UploadLimits.FormatMaxFileSizeMessage` — API errors always match enforced cap |
| **BE-U4** | `BeDemo.Api/Utils/UploadPathSecurity.cs` — segment validation + `Path.GetFullPath` containment under `wwwroot` |
| **Call sites** | `ProfileController.SaveAvatarFile`, `StoriesController` image POST |
| **BE-U3** | `UploadSignedUrlService` + `UploadsController` — clients receive time-limited HMAC URLs; bare `/uploads/*` returns 404/403 (not anonymously served) |
| **BE-U5** (partial) | Story images: `{Guid:N}{ext}`; avatars: fixed `global` / `face_{id}` names per user folder |

**Tests:** `UploadPathSecurityTests`, `UploadSecurityEdgeTests`, `UploadSignedUrlTests`.

**Config:** `Uploads:SigningSecret` (≥32 chars), `Uploads:SignedUrlLifetimeMinutes` (default 60). Override in production via `Uploads__SigningSecret`.

**Threat model:** Signed URLs are bearer capabilities until `exp`; anyone with the URL can download until expiry (same class as S3 presigned GET). Rotate `SigningSecret` to invalidate all outstanding links.
