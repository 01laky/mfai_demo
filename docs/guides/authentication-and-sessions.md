...
| Malformed JSON / bad model (controller)                           | **400** | `invalid_request` or validation errors        | Token body: FluentValidation via manual `ValidateAsync` → **`invalid_request`** (not `ValidationProblemDetails`). See [`api-request-validation.md`](./api-request-validation.md). |
