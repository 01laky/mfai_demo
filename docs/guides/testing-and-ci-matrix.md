...
| Java (`many_faces_mailer` mailer-worker) | `many_faces_mailer/` | `./gradlew test` (CI job `java_many_faces_mailer`, compose check `infra_many_faces_mailer`). gRPC + Mailpit dev stack — see [`mailer-local-dev.md`](./mailer-local-dev.md) and [`many_faces_mailer/README.md`](../../many_faces_mailer/README.md). |
