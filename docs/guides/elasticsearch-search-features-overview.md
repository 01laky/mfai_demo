...
| **Optional Docker TLS smoke** | **`many_faces_elastic/scripts/smoke-grpc-tls.sh`** — OpenSSL demo CA + server + client PEMs, **chmod** so **nonroot** distroless can read bind mounts, **`docker-compose.tls-smoke.yml`** (host ports **59210** / **59211**), **grpcurl** `Ping`, then **`SearchWorkerTlsEndToEndSmokeTests`** when `SEARCH_TLS_SMOKE=1`. |
